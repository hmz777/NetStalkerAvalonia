...
namespace NetStalkerAvalonia.Services.Implementations.AppLocking
{
	public class AppLockManagerLinux : ReactiveObject, IAppLockService
	{
		...
		{
			var passBytes = Tools.StringEncrypt(newPassword, key: Encoding.UTF8.GetBytes(_vaultKey));
		...
		private byte[]? GetDecryptedBytes()
		{
			...
			return Tools.StringDecrypt(encPassBytes, key: Encoding.UTF8.GetBytes(_vaultKey));
		}
